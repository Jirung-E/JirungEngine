...
BoxCollider::BoxCollider(const Point& top_left, const Point& buttom_right) : 
    ...
    side {
        Collider { top_left, Vector(buttom_right.x - top_left.x, 0) }, 
        Collider { Point(buttom_right.x, top_left.y), Vector(0, buttom_right.y - top_left.y) }, 
        Collider { buttom_right, Vector(top_left.x - buttom_right.x, 0) }, 
        Collider { Point(top_left.x, buttom_right.y), Vector(0, top_left.y - buttom_right.y) }
    } {
...
void BoxCollider::setTopLeftPoint(const Point& point) {
    top_left = point;
    side[3] = Collider { getButtomLeftPoint(), Vector(point.x - getButtomLeftPoint().x, point.y - getButtomLeftPoint().y) };
...
void BoxCollider::setTopRightPoint(const Point& point) {
    ...
    side[1] = Collider { point, Vector(getButtomRightPoint().x - point.x, getButtomRightPoint().y - point.x) };
}
...
void BoxCollider::setButtomRightPoint(const Point& point) {
    ...
    side[2] = Collider { buttom_right, Vector(getButtomLeftPoint().x - point.x, getButtomLeftPoint().y - point.x) };
}

void BoxCollider::setButtomLeftPoint(const Point& point) {
    buttom_left = point;
    side[2] = Collider { getButtomRightPoint(), Vector(point.x - getButtomRightPoint().x, point.y - getButtomRightPoint().y) };
